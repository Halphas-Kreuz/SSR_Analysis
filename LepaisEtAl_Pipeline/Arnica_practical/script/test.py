...
folder_path = '../filtered_results/filtered_stuttermark'
LocusCoverage_File = '../LocusCoverageperIndividual_nSSR_FullLength.csv'

def list_sample_names(folder_path, output_txt="sample_names.txt"):
    """
    Lists all CSV files in the folder (without suffix) and saves them to a text file.
    """
    sample_names = [os.path.splitext(file_name)[0] for file_name in os.listdir(folder_path) if file_name.endswith('.csv')]
    with open(output_txt, 'w') as file:
        file.write("\n".join(sample_names))
    return sample_names

sample_name_list = list_sample_names(folder_path)
...
def process_sample(folder_path):
    ...
    sample_file= search_folder(folder_path, test_sample_name)
